...
namespace M101DotNet.WebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> CommentLike(CommentLikeModel model)
        {
            ...
            var blogContext = new BlogContext();
            var fieldDefinition = String.Format("Comments.{0}.Likes", model.Index);
            var updateDefinition = Builders<Post>.Update.Inc(fieldDefinition, 1);

            await blogContext.Posts.UpdateOneAsync(p => p.Id == model.PostId, updateDefinition);
